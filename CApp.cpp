...
bool CApp::loadMedia()
{
    ...
    //Load music
    gMusic = Mix_LoadMUS( "One Final Effort (2).mp3" );
    if( gMusic == NULL ){
    ...
    //Load sound effects
    gScratch = Mix_LoadWAV( "Wilhelm Scream.mp3" );
    if( gScratch == NULL ){
        printf( "Failed to load death sound effect! SDL_mixer Error: %s\n", Mix_GetError() );
        Running = false;
    }

    gSave = Mix_LoadWAV("Rocket launch.mp3");
    if (gSave == NULL) {
        printf("Failed to load save sound effect! SDL_mixer Error: %s\n", Mix_GetError());
        Running = false;
    }

    gGameOver = Mix_LoadWAV("gameover.mp3");
    if (gGameOver == NULL) {
